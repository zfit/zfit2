...
class WeightedMean(Accumulator):
    ...
    def mean(self) -> jnp.ndarray:
        ...
        return jnp.where(
            ...
            self.weighted_sum / self.sum_of_weights,
            0.0
        )
...
class Hist:
        ...
        *args: Union[axis.Axis, Tuple[int, float, float]],
        ...
        for arg in args:
            if isinstance(arg, tuple) and len(arg) == 3:
                ...
                raise TypeError(f"Expected Axis or (bins, start, stop) tuple, got {type(arg)}")
    ...
    def fill(
        ...
        if all(np.isscalar(val) for val in values):
            ...
            indices = tuple(int(self.axes[i].index(val)) for i, val in enumerate(values))
    ...
    def __getitem__(
        ...
        new_axes = []
        for i, (k, ax) in enumerate(zip(key, self.axes)):
    ...
    def project(self, *axes: Union[int, str]) -> Hist:
        ...
        result = Hist(*new_axes, name=self.name, label=self.label, metadata=self.metadata.copy())
    ...
    def sum(self, flow: bool = True) -> float:
        ...
        if not flow:
            slices = tuple(slice(1, -1) if ax.has_underflow and ax.has_overflow else
                          slice(None) for ax in self.axes)
    ...
    def profile(self, axis: Union[int, str]) -> Hist:
        ...
        result = Hist(*profiled_axes, name=self.name, label=self.label, metadata=self.metadata.copy())
...
class NamedHist(Hist):
    ...
    def __getitem__(
        self, 
        key: Dict[str, Union[int, slice]]
    ) -> Union[NamedHist, float]:
