...
class JaxHist:
        ...
        *axes: Axis,
        ...
        if counts is not None:
            if counts.shape != shape:
                raise ValueError(f"Counts shape {counts.shape} doesn't match expected shape {shape}")
            self._counts = counts
        else:
    ...
    def fill(self, values: List[jnp.ndarray], weights: Optional[jnp.ndarray] = None) -> JaxHist:
        ...
        def add_point(counts, i):
            indices = tuple(ax.index(arr[i % arr.shape[0]]) for ax, arr in zip(self.axes, arrays))
        ...
        new_counts = jax.lax.fori_loop(
            0, max_len,
            lambda i, counts: add_point(counts, i),
            self._counts
        )
    ...
    def fill_np(self, values: List[np.ndarray], weights: Optional[np.ndarray] = None) -> JaxHist:
        ...
        for i in range(max_len):
            try:
                indices = tuple(int(ax.index(arr[i % arr.shape[0]])) for ax, arr in zip(self.axes, arrays))
        ...
        # Create a new histogram with updated counts
        return JaxHist(*self.axes, counts=jnp.array(counts), name=self.name, label=self.label)
    ...
    def __truediv__(self, other: Union[JaxHist, float]) -> JaxHist:
        ...
        if isinstance(other, JaxHist):
            ...
            new_counts = jnp.where(other._counts != 0, self._counts / other._counts, 0.0)
        else:
            new_counts = self._counts / other
...
def _hist_unflatten(aux_data, children):
    ...
    counts, = children
    return JaxHist(*aux_data["axes"], counts=counts, name=aux_data["name"], label=aux_data["label"])
