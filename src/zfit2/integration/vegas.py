...
class VegasIntegrator:
    ...
    def _sample_and_evaluate(
        self, key: jnp.ndarray, n_samples: int, edges: list[jnp.ndarray], **kwargs
    ...
    def integrate(
        self, n_samples: int = 10000, n_iterations: int = 10, **kwargs
    ) -> tuple[float, float]:
...
def integrate_vegas(
    ...
    **kwargs,
    ...
    integrator = VegasIntegrator(
        func=func, limits=limits, n_bins=n_bins, alpha=alpha, beta=beta, seed=seed
    )

    # Perform integration
    result, error = integrator.integrate(
        n_samples=n_samples, n_iterations=n_iterations, **kwargs
    )
