...
def integrate_trapezoid(
    ...
    **kwargs
    ...
    h = (upper - lower) / (n_points - 1)
    return h * (0.5 * func_values[0] + jnp.sum(func_values[1:-1]) + 0.5 * func_values[-1])
...
def integrate_monte_carlo(
    ...
    **kwargs
    ...
    for i, (lower, upper) in enumerate(limits):
        ...
        points.append(jax.random.uniform(
            subkey, shape=(n_points,), minval=lower, maxval=upper
        ))
    
    # Evaluate function at random points
    func_values = jnp.array([func(*[points[d][i] for d in range(ndim)], **kwargs) 
