...
class NumPyBackend(BackendBase):
    ...
    def _finite_diff_hessian(self, func, x, eps=1e-5):
        ...
        for i in range(n):
            for j in range(i, n):
                ...
                hess_val = (func(x_pp) - func(x_pm) - func(x_mp) + func(x_mm)) / (4 * eps * eps)
    ...
    def custom_jvp(self, fun: Callable, jvp: Optional[Callable] = None) -> Callable:
        ...
        if jvp is not None:
            import warnings
            warnings.warn("Custom JVP rules are not supported in NumPy backend", UserWarning)
        return fun
    ...
    def custom_vjp(self, fun: Callable, fwd: Optional[Callable] = None, bwd: Optional[Callable] = None) -> Callable:
        ...
        if fwd is not None or bwd is not None:
            import warnings
            warnings.warn("Custom VJP rules are not supported in NumPy backend", UserWarning)
        return fun
    ...
    def tree_unflatten(self, treedef: Any, leaves: List[Any]) -> Any:
        ...
        def unflatten_helper(path, treedef_list):
            ...
            for td_path, td_type, td_info in treedef_list:
                if td_path[:len(path)] == path and len(td_path) == len(path) + 1:
                    ...
                    if td_type == "leaf":
                        ...
                        subtree_treedef = [t for t in treedef_list if t[0][:len(td_path)] == td_path]
            ...
            for td_path, td_type, td_info in treedef_list:
                if td_path == path:
                    if td_type == list:
                        result = [path_dict.get(i, None) for i in range(td_info)]
                    elif td_type == tuple:
                        result = tuple(path_dict.get(i, None) for i in range(td_info))
                    elif td_type == dict:
                        result = {k: path_dict.get(k, None) for k in td_info}
                    elif td_type == "leaf":
                        result = path_dict.get(0, None)
                    break
    ...
    def vmap(self, fun: Callable, in_axes=0, out_axes=0) -> Callable:
        ...
        def vectorized_fun(*args):
            ...
            if len(actual_in_axes) != len(args):
                raise ValueError(f"Length of in_axes ({len(actual_in_axes)}) must match number of arguments ({len(args)})")
            
            # Determine the shape of the output by broadcasting the inputs along their respective axes
            broadcast_shape = None
            for arg, in_ax in zip(args, actual_in_axes):
                if in_ax is not None:
                    ...
                    if arg_array.ndim > in_ax:
                        ...
                        if broadcast_shape is None:
                            ...
                            raise ValueError(f"Mismatched shapes for vectorization: {broadcast_shape} vs {axis_shape}")
            ...
            first_result = fun(*(arg[0] if in_ax == 0 and np.asarray(arg).ndim > 0 else arg 
                                for arg, in_ax in zip(args, actual_in_axes)))
    ...
    def pmap(self, fun: Callable, axis_name=None, devices=None) -> Callable:
        ...
        """
        import warnings
        warnings.warn("pmap is not parallelized in NumPy backend, using serial execution", UserWarning)
        
        def pmapped_fun(*args):
            # Assume first dimension is the one to map over
            results = []
            for slices in zip(*(arg for arg in args)):
