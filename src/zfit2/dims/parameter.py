...
class Parameters(Variables):
    ...
    def values(self):
        ...
        return jnp.array([param.value for param in self.params])

    def fixed(self):
        """Return the fixed status of the parameters."""
        return jnp.array([param.fixed for param in self.params])

    def stepsizes(self):
        """Return the stepsizes of the parameters."""
        # Convert to numpy array with float64 dtype to match test expectations
        return jnp.array([param.stepsize for param in self.params], dtype=jnp.float64)
...
# JAX PyTree registration for Parameters class
def _parameters_flatten(params_obj: Parameters) -> tuple[list[Parameter], None]:
    """Flatten a Parameters object for JAX PyTree."""
    # The individual parameters are the children
    children = params_obj.params
    # No auxiliary data needed
    aux_data = None
    return children, aux_data


def _parameters_unflatten(aux_data: None, children: list[Parameter]) -> Parameters:
    """Unflatten a Parameters object from JAX PyTree."""
    return Parameters(children)


# Register Parameters class with JAX
jax.tree_util.register_pytree_node(
    Parameters, _parameters_flatten, _parameters_unflatten
)
