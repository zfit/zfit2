...
def test_model_pdf():
    ...
    np.testing.assert_allclose(actual_pdf_custom, expected_pdf_custom, rtol=1e-5)
...
def test_model_integration():
    ...
    np.testing.assert_allclose(result1, 1.0, rtol=1e-5)  # Normalized, should be 1.0
    ...
    np.testing.assert_allclose(result3, expected3, rtol=1e-5)
...
def test_gaussian_model():
    ...
    np.testing.assert_allclose(pdf_at_mean, expected_pdf, rtol=1e-2)

    # Test integration over a symmetric range
    result = model.integrate([(-5.0, 5.0)])
    # Should be very close to 1 (about 0.9973)
    np.testing.assert_allclose(result, 0.9973, rtol=1e-2)
...
def test_polynomial_model():
    ...
    np.testing.assert_allclose(pdf, expected_pdf, rtol=1e-5)

    # Test integration
    result = model.integrate([(0.0, 1.0)])
    np.testing.assert_allclose(result, 1.0, rtol=1e-5)  # Normalized, should be 1.0
