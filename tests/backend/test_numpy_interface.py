...
def test_array_creation():
    ...
    d = znp.full((2, 3), 5)
    assert d.shape == (2, 3)
    assert np.all(np.array(d) == 5)
    
    # Test zeros_like, ones_like, full_like
    e = znp.zeros_like(a)
    assert e.shape == a.shape
    assert np.all(np.array(e) == 0)
    
    f = znp.ones_like(a)
    assert f.shape == a.shape
    assert np.all(np.array(f) == 1)
    
    g = znp.full_like(a, 5)
    assert g.shape == a.shape
    assert np.all(np.array(g) == 5)
    
    # Test arange and linspace
    h = znp.arange(5)
    assert np.array_equal(np.array(h), np.array([0, 1, 2, 3, 4]))
    
    i = znp.linspace(0, 10, 6)
    assert np.allclose(np.array(i), np.array([0, 2, 4, 6, 8, 10]))
    
    # Test eye and identity
    j = znp.eye(3)
    assert j.shape == (3, 3)
    assert np.array_equal(np.array(j), np.eye(3))
    
    k = znp.identity(3)
    assert k.shape == (3, 3)
    assert np.array_equal(np.array(k), np.identity(3))
...
def test_array_manipulation():
    ...
    b = znp.reshape(a, (3, 2))
    assert b.shape == (3, 2)
    assert np.array_equal(np.array(b), np.array([[1, 2], [3, 4], [5, 6]]))
    
    # Test transpose
    c = znp.transpose(a)
    assert c.shape == (3, 2)
    assert np.array_equal(np.array(c), np.array([[1, 4], [2, 5], [3, 6]]))
    
    # Test concatenate
    d1 = znp.array([1, 2, 3])
    d2 = znp.array([4, 5, 6])
    d = znp.concatenate([d1, d2])
    assert np.array_equal(np.array(d), np.array([1, 2, 3, 4, 5, 6]))
    
    # Test stack
    e = znp.stack([d1, d2])
    assert e.shape == (2, 3)
    assert np.array_equal(np.array(e), np.array([[1, 2, 3], [4, 5, 6]]))
    
    # Test vstack and hstack
    f = znp.vstack([d1, d2])
    assert f.shape == (2, 3)
    assert np.array_equal(np.array(f), np.array([[1, 2, 3], [4, 5, 6]]))
    
    g = znp.hstack([znp.array([[1], [2]]), znp.array([[3], [4]])])
    assert g.shape == (2, 2)
    assert np.array_equal(np.array(g), np.array([[1, 3], [2, 4]]))
    
    # Test where
    condition = znp.array([True, False, True])
    h = znp.where(condition, d1, d2)
    assert np.array_equal(np.array(h), np.array([1, 5, 3]))
...
def test_math_operations():
    ...
    # Test basic math
    assert np.array_equal(np.array(a + 1), np.array([2, 3, 4]))
    assert np.array_equal(np.array(a * 2), np.array([2, 4, 6]))
    assert np.array_equal(np.array(a / 2), np.array([0.5, 1, 1.5]))
    ...
    assert np.allclose(np.array(znp.exp(a)), np.exp(np.array(a)))
    assert np.allclose(np.array(znp.log(a)), np.log(np.array(a)))
    assert np.allclose(np.array(znp.sin(a)), np.sin(np.array(a)))
    assert np.allclose(np.array(znp.cos(a)), np.cos(np.array(a)))
    assert np.allclose(np.array(znp.sqrt(a)), np.sqrt(np.array(a)))
    ...
    c = znp.array([[1, 2], [3, 4]])
    d = znp.array([[5, 6], [7, 8]])
    assert np.array_equal(np.array(znp.matmul(c, d)), np.matmul(np.array(c), np.array(d)))
...
def test_comparison_functions():
    ...
    assert np.array_equal(np.array(result), np.array([True, False, False]))
    
    # Test greater
    result = znp.greater(a, b)
    assert np.array_equal(np.array(result), np.array([False, False, True]))
    
    # Test less
    result = znp.less(a, b)
    assert np.array_equal(np.array(result), np.array([False, True, False]))
...
def test_linalg_functions():
    ...
    assert np.allclose(np.array(identity), np.eye(2), rtol=1e-5)
    ...
    assert np.allclose(eigvals_sorted, expected, rtol=1e-5)
    ...
    try:
        ...
        assert np.allclose(np.array(a_reconstructed), np.array(a), rtol=1e-5)
...
def test_lazy_loading():
    ...
    try:
        ...
        assert np.array_equal(np.array(result), np.array([1, 3, 6]))
